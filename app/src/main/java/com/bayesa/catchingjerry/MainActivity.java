package com.bayesa.catchingjerry;

import static java.lang.Integer.parseInt;

import androidx.appcompat.app.AlertDialog;
import androidx.appcompat.app.AppCompatActivity;

import android.content.Context;
import android.content.DialogInterface;
import android.content.Intent;
import android.content.SharedPreferences;
import android.os.Bundle;
import android.os.CountDownTimer;
import android.os.Handler;
import android.view.View;
import android.widget.ImageView;
import android.widget.TextView;
import android.widget.Toast;

import java.util.Random;

public class MainActivity extends AppCompatActivity {
    Runnable runnable;
    Handler handler;

    SharedPreferences sharedPreferences;

    ImageView[] imageArray;
    ImageView imageView1;
    ImageView imageView2;
    ImageView imageView3;
    ImageView imageView4;
    ImageView imageView5;
    ImageView imageView6;
    ImageView imageView7;
    ImageView imageView8;
    ImageView imageView9;
    TextView timeTextView;
    TextView scoreTextView;
    int score;
    int high_score;
    int storedData;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_main);
        // initialize ImageView
        imageView1 = findViewById(R.id.imageView2);
        imageView2 = findViewById(R.id.imageView3);
        imageView3 = findViewById(R.id.imageView4);
        imageView4 = findViewById(R.id.imageView5);
        imageView5 = findViewById(R.id.imageView6);
        imageView6 = findViewById(R.id.imageView7);
        imageView7 = findViewById(R.id.imageView8);
        imageView8 = findViewById(R.id.imageView9);
        imageView9 = findViewById(R.id.imageView10);


        // initialize TextView
        timeTextView = findViewById(R.id.timeTextView);
        scoreTextView = findViewById(R.id.scoreTextView);

        score = 0;
        high_score = 0;

        sharedPreferences = this.getSharedPreferences("com.bayesa.catchingjerry", Context.MODE_PRIVATE);
        high_score = sharedPreferences.getInt("highScore", 0 );

        imageArray = new ImageView[]{imageView1,imageView2,imageView3, imageView4, imageView5, imageView6, imageView7, imageView8, imageView9};
        hideImage();

        new CountDownTimer(20000,1000) {
            @Override
            public void onTick(long l) {
                timeTextView.setText("Time: " + l/1000);
            }

            @Override
            public void onFinish() {
                timeTextView.setText("Finished");
                handler.removeCallbacks(runnable);

                for(ImageView image : imageArray){
                    image.setVisibility(View.INVISIBLE);
                }

                AlertDialog.Builder alert = new AlertDialog.Builder(MainActivity.this);
                alert.setTitle("Restart Game");
                alert.setMessage("Are you sure?");
                alert.setPositiveButton("Yes", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialogInterface, int i) {
                        reload();
                    }
                });
                alert.setNegativeButton("No", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialogInterface, int i) {

                        storedData = sharedPreferences.getInt("score",0);

                        Intent intent = new Intent(MainActivity.this, ScorePage.class);
                        // putExtra includes send data.
                        intent.putExtra("score",storedData); // key, value
                        intent.putExtra("highScore", high_score);
                        startActivity(intent);
                        // Data is restarted in "sharedPreferences" before the game is restarted.
                        sharedPreferences.edit().remove("score").apply();
                    }
                });

                if (score > high_score) {
                    sharedPreferences.edit().putInt("highScore",score).apply();
                }
                alert.show();

                // get current score






            }
        }.start();
    }

    public void increaseScore(View view) {
        score++;
        scoreTextView.setText("Score: " + score);
        sharedPreferences.edit().putInt("score", score).apply();
        sharedPreferences.edit().putInt("highScore",high_score).apply();

    }

    public void hideImage(){

        handler = new Handler();
        runnable = new Runnable() {
            @Override
            public void run() {
                for(ImageView image : imageArray){
                    image.setVisibility(View.INVISIBLE);
                }

                Random random = new Random();
                int i = random.nextInt(9);
                imageArray[i].setVisibility(View.VISIBLE);
                handler.postDelayed(this, 750);

            }
        };
        handler.post(runnable);



    }

    public void reload() {
        Intent intent = getIntent();
        overridePendingTransition(0, 0);
        intent.addFlags(Intent.FLAG_ACTIVITY_NO_ANIMATION);
        finish();
        overridePendingTransition(0, 0);
        startActivity(intent);
    }

    public void showAlert(View view){
        AlertDialog.Builder alert = new AlertDialog.Builder(MainActivity.this);
        alert.setTitle("Restart Game");
        alert.setMessage("Are you sure?");
        alert.setPositiveButton("Yes", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialogInterface, int i) {
                reload();
            }
        });
        alert.setNegativeButton("No", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialogInterface, int i) {
                int storedData = sharedPreferences.getInt("score",0);
                Intent intent = new Intent(MainActivity.this, ScorePage.class);
                // putExtra includes send data.
                intent.putExtra("score",storedData); // key, value
                intent.putExtra("highScore", high_score);
                startActivity(intent);
                // Data is restarted in "sharedPreferences" before the game is restarted.
                sharedPreferences.edit().remove("score").apply();
            }
        });
        if (score > high_score) {
            sharedPreferences.edit().putInt("highScore",score).apply();
        }
        alert.show();


        alert.show();
    }


}